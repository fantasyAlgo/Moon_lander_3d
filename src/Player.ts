...
export class Player extends Shape {
  ...
  update(moveVec : Vec3, camera : Camera, dt : number){
    //this.vel = Vec3.add(this.vel, Vec3.multScalar(moveVec, 0.001*dt));
    const moveMatrix = Mat4x4.T(Mat4x4.LookAtRH(Vec3.make(0,0,0), camera.forward, UP_VEC));
    const newMoveVec : Vec4 = Mat4x4.multVec4(moveMatrix, Vec4.make(moveVec.x, 0, moveVec.z, 1.0)); 

    if (moveVec.y > 0) this.vel.y += 0.0005*dt;
    this.vel = Vec3.add(this.vel, Vec3.multScalar(Vec3.make(newMoveVec.x, 0, newMoveVec.z), 0.0005*dt));
    ...
    this.rotationAxis = Vec3.add(this.rotationAxis, Vec3.multScalar(subY, 0.002*dt));
